...
export default function Receipt({ sale, formatPeso, formatDate, getSaleStatusLabel, getAppointmentBarberName }: ReceiptProps) {
  if (!sale) return null;
  ...
  const vatAmount = Math.max(Number(sale.vatAmount || 0), Number(sale.payment?.vatAmount || 0));
  ...
  return (
    <>
      ...
      <article className="print-receipt">
        ...
        <section className="print-receipt__section" style={{ margin: "12px 0" }}>
          <ReceiptRow label="Subtotal" value={formatPeso(sale.subtotal)} />
          <ReceiptRow label={vatExempt ? "VAT (Exempt)" : "VAT"} value={vatExempt ? "VAT Exempt" : formatPeso(vatAmount)} />
          ...
          <ReceiptRow label="Total Payment" value={formatPeso(sale.totalAmount)} />
