    ...
    <Box>
      ...
      <Box sx={{ border: '1px solid #d0d0d0', borderRadius: 2, overflow: 'hidden', bgcolor: '#fff' }}>
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
          {WEEKDAYS.map((d) => (
            <Box key={d} sx={{
              textAlign: 'center', py: 0.8, fontSize: 11, fontWeight: 600, color: '#666',
              borderRight: '1px solid #e0e0e0', borderBottom: '1px solid #d0d0d0',
          ...
          {calDays.map((day, index) => {
            if (!day) return (
              <Box key={`empty-${index}`} sx={{
                minHeight: 36, borderRight: '1px solid #e0e0e0', borderBottom: '1px solid #e0e0e0',
                bgcolor: '#f0f0f0', '&:nth-of-type(7n)': { borderRight: 'none' },
              }} />
            );
            ...
            return (
              <Box key={index} onClick={() => onSelectDay(dt)} sx={{
                minHeight: 36, borderRight: '1px solid #e0e0e0', borderBottom: '1px solid #e0e0e0',
                p: '6px 6px 4px', bgcolor: bgColor, cursor: 'pointer',
...
function StatCard({ icon, value, label, trend }: { icon: string; value: string; label: string; trend: number }) {
  ...
  return (
    <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 2, bgcolor: '#fff' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
        ...
        <Typography sx={{ fontSize: 12, color: positive ? '#16a34a' : '#dc2626', fontWeight: 600 }}>
          {positive ? '↑' : '↓'} {Math.abs(trend)}%
        </Typography>
      </Box>
      <Typography sx={{ fontSize: 24, fontWeight: 700, color: '#111', lineHeight: 1.1 }}>{value}</Typography>
      <Typography sx={{ fontSize: 12, color: '#888', mt: 0.5 }}>{label}</Typography>
    </Box>
  );
}
...
function InsightCard({ icon, title, body }: InsightCard) {
  return (
    <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 2.5, bgcolor: '#fff' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
        <Typography sx={{ fontSize: 22 }}>{icon}</Typography>
        <Typography sx={{ fontWeight: 700, fontSize: 14 }}>{title}</Typography>
      </Box>
      <Typography sx={{ fontSize: 13, color: '#555', lineHeight: 1.6 }}>{body}</Typography>
    </Box>
  );
}
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <Box sx={{ bgcolor: '#fff', border: '1px solid #d0d0d0', borderRadius: 1, p: 1.5, fontSize: 12 }}>
      <Typography sx={{ fontWeight: 700, mb: 0.5, fontSize: 12 }}>{label}</Typography>
      <Typography sx={{ fontSize: 12 }}>Revenue: {payload[0]?.value?.toLocaleString()}</Typography>
      <Typography sx={{ fontSize: 12 }}>Transactions: {payload[1]?.value}</Typography>
    </Box>
  );
}
...
export default function ReportsPage() {
  ...
  const handleSelectDay = (clicked: Date) => {
    if (picking === 'start' || (startDate && endDate)) {
      setStartDate(clicked); setEndDate(null); setPicking('end');
    } else {
      if (startDate && clicked < startDate) { setEndDate(startDate); setStartDate(clicked); }
  ...
  try {
    // Step 1: fetch real DB data
    const dataRes = await fetch(
      `/api/admin/reports/data?from=${toISO(startDate)}&to=${toISO(endDate)}`
    );
    ...
    // Step 2: GPT-4o-mini for insights/trends JSON
    ...
    const PIE_COLORS = ['#4285F4', '#FBBC05', '#EA4335', '#34A853', '#AB47BC'];
    const services = (rawData.services ?? []).map((s: any, i: number) => ({
      ...s,
  ...
  if (reportData) {
    const pieData = reportData.services.map(s => ({ name: s.name, value: s.revenue }));
    ...
    return (
      <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 960, mx: 'auto' }}>
        ...
        <Typography sx={{ fontWeight: 700, fontSize: 16, mt: 2, mb: 0.3 }}>AI Business Data Report</Typography>
        <Typography sx={{ fontSize: 13, color: '#888', mb: 0.5 }}>{reportData.dateRange}</Typography>
        <Typography
          onClick={handleExportCSV}
          sx={{ fontSize: 13, color: '#4285F4', cursor: 'pointer', mb: 2.5, display: 'inline-block' }}
        >
          Export CSV
        </Typography>

        {/* Insight cards */}
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: 'repeat(3, 1fr)' }, gap: 2, mb: 2.5 }}>
          {reportData.insights.map((ins, i) => <InsightCard key={i} {...ins} />)}
        </Box>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2, mb: 2.5 }}>
          {/* Line chart */}
          <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 2.5, bgcolor: '#fff' }}>
            <Typography sx={{ fontWeight: 700, fontSize: 14, mb: 2 }}>Revenue and Transaction Trend</Typography>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={reportData.weeklyData}>
                ...
                <Line type="monotone" dataKey="transactions" stroke="#FBBC05" strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
            <Box sx={{ bgcolor: '#f9f9f9', border: '1px solid #e8e8e8', borderRadius: 1, p: 1.5, mt: 1.5 }}>
          ...
          {/* Pie chart + table */}
          <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 2.5, bgcolor: '#fff' }}>
            <Typography sx={{ fontWeight: 700, fontSize: 14, mb: 1 }}>Service Revenue Distribution</Typography>
            ...
            {/* Service table */}
            <Box sx={{ mt: 1 }}>
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 1, pb: 0.5, borderBottom: '1px solid #eee', mb: 0.5 }}>
                {['Service', 'Count', 'Revenue'].map(h => (
                  <Typography key={h} sx={{ fontSize: 11, fontWeight: 600, color: '#888' }}>{h}</Typography>
                ))}
              </Box>
              {reportData.services.map((s, i) => (
                <Box key={i} sx={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 1, py: 0.5 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
                    <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: s.color, flexShrink: 0 }} />
                    <Typography sx={{ fontSize: 12 }}>{s.name}</Typography>
                  </Box>
                  <Typography sx={{ fontSize: 12, textAlign: 'right' }}>{s.count}</Typography>
                  <Typography sx={{ fontSize: 12, textAlign: 'right' }}>₱{s.revenue.toLocaleString()}</Typography>
                </Box>
              ))}
            </Box>
            ...
            <Box sx={{ bgcolor: '#f9f9f9', border: '1px solid #e8e8e8', borderRadius: 1, p: 1.5, mt: 1.5 }}>
              <Typography sx={{ fontSize: 12, color: '#555' }}>
                <strong>AI Recommendation:</strong> {reportData.serviceRecommendation}
              </Typography>
            </Box>
          </Box>
        </Box>
        ...
        <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, overflow: 'hidden' }}>
          ...
          {/* Chat body */}
          <Box sx={{ bgcolor: '#fff', px: 2.5, pt: 2, pb: 1, minHeight: 220, maxHeight: 340, overflowY: 'auto' }}>
            {/* Quick questions */}
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.8, mb: 2 }}>
              {QUICK_QUESTIONS.map((q) => (
                <Box
                  ...
                  sx={{
                    fontSize: 12, bgcolor: '#111', color: '#FBBC05', px: 1.5, py: 0.6,
            ...
            {chatMsgs.map((m, i) => (
              <Box key={i} sx={{ mb: 1.5, display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                <Box sx={{
                  maxWidth: '80%', px: 2, py: 1.2, borderRadius: 2, fontSize: 13, lineHeight: 1.6,
                  ...
                  '& h1, & h2, & h3': { fontSize: 13, fontWeight: 700, mt: 1, mb: 0.5 },
                  '& p:last-child, & ul:last-child, & ol:last-child': { mb: 0 },
                }}>
                  {m.role === 'assistant' ? (
            ...
            {chatLoading && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                ...
                <Typography sx={{ fontSize: 12, color: '#888' }}>Thinking…</Typography>
              </Box>
            )}
            <div ref={chatEndRef} />
          </Box>
          ...
          <Box sx={{
            ...
            <input
              ...
              style={{
                ...
                outline: 'none', fontSize: 13, color: '#222',
              }}
            />
            ...
            <Button
              ...
              startIcon={chatLoading ? <CircularProgress size={12} sx={{ color: '#111' }} /> : <AutoAwesomeIcon sx={{ fontSize: 14 }} />}
              sx={{
                fontSize: 11, fontWeight: 600, textTransform: 'none',
                color: '#111', bgcolor: '#FBBC05', px: 1.5, py: 0.6,
                borderRadius: 5, minWidth: 'unset', whiteSpace: 'nowrap',
            ...
            <IconButton
              ...
              disabled={chatLoading || !chatInput.trim()}
            >
              <SendIcon fontSize="small" sx={{ color: chatInput.trim() ? '#111' : '#bbb' }} />
            </IconButton>
          </Box>
        </Box>
      </Box>
    ...
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 900, mx: 'auto' }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2, mb: 2.5 }}>
        {[
          ...
          <Box key={label} sx={{
            bgcolor: '#f4f4f4', border: '1px solid #e0e0e0', borderRadius: 2,
            px: 2, py: 1.2, minHeight: 42, display: 'flex', alignItems: 'center',
          }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3, mb: 3 }}>
        <Calendar monthDate={leftMonth} startDate={startDate} endDate={endDate}
          onNavigate={(dir) => handleNavigate('left', dir)} onSelectDay={handleSelectDay} />
        <Calendar monthDate={rightMonth} startDate={startDate} endDate={endDate}
          onNavigate={(dir) => handleNavigate('right', dir)} onSelectDay={handleSelectDay} />
      </Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        <Button
          ...
          sx={{
            bgcolor: '#111', color: '#fff', px: 5, py: 1.4, fontSize: 14, fontWeight: 600,
